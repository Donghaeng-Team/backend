package com.bytogether.marketservice.controller;

import com.bytogether.marketservice.service.MarketFacadeService;
import lombok.RequiredArgsConstructor;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PathVariable;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

/**
 * 공동 구매 마켓 관련  public API 컨트롤러
 *
 * @author insu9058@naver.com
 * @version 1.0
 * @since 2025-09-30
 */

@RestController
@RequestMapping("/api/v1/market/public")
@RequiredArgsConstructor
public class MarketPublicController {
    private final MarketFacadeService marketFacadeService;
//    1.마켓글 목록 조회 (필터링, 페이징) getMarketPosts - public
//    2.마켓글 상세 조회 getMarketPostDetail - public
//    3.특정 유저의 마켓글 검색 getPostsByUserId - public

    // 1. 마켓글 목록 조회 (필터링, 페이징) getMarketPosts - public
    @GetMapping
    public String getMarketPosts() {
        return "List of market posts";
    }

    // 2. 마켓글 상세 조회 getMarketPostDetail - public
    @GetMapping("/{marketId}")
    public String getMarketPostDetail(@PathVariable Long marketId) {
        return "Details of market post " + marketId;
    }

    // 3. 특정 유저의 마켓글 검색 getPostsByUserId - public
    @GetMapping("/user/{userId}")
    public String getPostsByUserId(@PathVariable("userId") Long targetUserId) {
        marketFacadeService.getMyMarketPosts(targetUserId);
        return "List of my market posts";
    }
}
